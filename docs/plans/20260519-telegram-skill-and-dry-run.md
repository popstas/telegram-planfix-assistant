...
- [ ] пройтись по всем командам из таблицы Resources & actions и зафиксировать, где `--dry-run` уже есть, а где его нет
- [ ] для каждой меняющей команды зафиксировать, какие шаги валидации она уже делает, и какие нужно перевести в безопасный режим
- [ ] определить общую структуру вывода `status = dry_run` (формат и поля), чтобы все команды отвечали единообразно
- [ ] зафиксировать список «технических аккаунтов» и `@planfix_bot`, которые нельзя удалять без `--force`
- [ ] write tests describing the expected shape of `--dry-run` output (общий контракт)
...
- [ ] создать файл `./skills/telegram-planfix-assistant/SKILL.md` с YAML front-matter и заголовком
- [ ] описать, где лежит конфиг (`data/config.yml`) и как им пользоваться (default folder и прочее)
- [ ] описать команду `telegram-planfix-assistant health` как проверку живости перед любыми изменениями
- [ ] зафиксировать правило: основной интерфейс агента — CLI, не прямые вызовы Telethon
- [ ] описать общий алгоритм агента из 11 шагов (читать просьбу → resource/action → параметры → health → CSV → dry-run → план → подтверждение → запуск → результат)
- [ ] прописать правила подготовки временных CSV/JSON в `/tmp`, не в репозитории
- [ ] прописать правила подтверждения для destructive- и массовых команд (кнопка «Выполнить?» либо текстовое подтверждение)
- [ ] write tests или проверки (например, lint/schema) для структуры SKILL.md, если в проекте есть подходящий механизм
- [ ] run project tests - must pass before next task

### Task 6: Описать в `SKILL.md` все resource/action и сценарии

- [ ] добавить таблицу Resources & actions со всеми 13 парами resource/action
- [ ] для каждой пары описать: какие данные извлекать из просьбы, какие флаги обязательны, какие можно брать из конфига, нужен ли временный CSV/JSON, что автоматизировать, где нужно подтверждение, какие типовые ошибки показывать
- [ ] добавить раздел сценария `groups create` с обезличенным примером команды (`--planfix-task-id`, `--title`, `--admin`, `--member`, `--folder-name`)
- [ ] добавить сценарий `topics create` (поиск чата по `--chat-name` + `--folder-name`, `--topic-name`)
- [ ] добавить сценарий `topics bulk-create` с примером временного CSV в `/tmp` и вызовом `--file`
- [ ] добавить сценарий `topics close` с обязательным планом и подтверждением
- [ ] добавить сценарий `members bulk-add` с временным CSV, обязательным `--dry-run`, затем повторным запуском без `--dry-run`
- [ ] добавить сценарий `members bulk-remove` с особыми правилами: всегда `--dry-run`, защита технических аккаунтов и `@planfix_bot`, `--force` только по явному подтверждению
- [ ] добавить сценарий `messages send` (одиночный и массовый по folder + topic name) с явным предупреждением о множественной рассылке
- [ ] добавить сценарий `folders inspect` как read-only без подтверждения
- [ ] добавить сценарий `folders add-chat` как изменение состояния с подтверждением
- [ ] добавить сценарии `operations status` (без подтверждения) и `operations retry` (только после просмотра статуса и подтверждения)
- [ ] описать `auth` как ручную команду для администратора — агент не запрашивает коды и пароли в чат
- [ ] write tests или smoke-проверку, что в `SKILL.md` присутствуют все 13 resource/action
- [ ] run project tests - must pass before next task
